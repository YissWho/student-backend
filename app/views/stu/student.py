...
def update_profile(request):
    ...
    if request.method == "GET":
        ...
        return Response(
            {"status": "success", "data": serializer.data, "code": 200},
    ...
    return Response(
        {"status": "false", "message": serializer.errors, "code": 400},
...
def get_classmates(request):
    ...
    try:
        ...
        # 查询同班同学，排除自己
        classmates = Student.objects.filter(classs=student.classs).exclude(  # 同班
            id=student.id  # 排除自己
        )
        ...
        return Response(
            {"status": "error", "message": "学生不存在", "code": 404},
            status=status.HTTP_404_NOT_FOUND,
        )
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if not student.check_password(serializer.validated_data["old_password"]):
                return Response(
                    {"status": "false", "message": "原密码错误", "code": 400},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"status": "success", "message": "密码修改成功", "code": 200},
                status=status.HTTP_200_OK,
            )
